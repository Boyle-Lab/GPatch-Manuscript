...
def main():
    ...
    cum_reflen = 0
    cum_pchlen = 0
    sys.stdout.write("chrom\tref_len\tpatched_len\tpct_coverage\traw_bag_dist\tnorm_bag_dist\tscaled_bag_dist\tnorm_scaled_bag_dist\tLevenshtein_dist\tnorm_Levenshtein_dist\n")
    for patched_fasta in SeqIO.parse(args.patched_fasta, "fasta"):
        ...
        if patched_fasta.id in reference_fasta:
            ...
            aln = edlib.align(str(ref_seq), str(patched_seq))
            ldist = aln['editDistance']
            norm_ldist = ldist / rlen
            sys.stdout.write("%s\t%d\t%d\t%f\t%d\t%f\t%d\t%f\t%d\t%f\n" % (ID, len(ref_seq), len(patched_seq), len(patched_seq)/len(ref_seq), raw_dist, norm_rdist, scaled_dist, norm_sdist, ldist, norm_ldist))
            cum_rdist += raw_dist
    ...
    sys.stdout.write("%s\t%d\t%d\t%f\t%d\t%f\t%d\t%f\t%d\t%f\n" % ("average", cum_reflen, cum_pchlen, cum_pchlen/cum_reflen, cum_rdist, cum_rdist/cum_rlen, cum_sdist, cum_sdist/cum_rlen, cum_ldist, cum_ldist/cum_rlen))
