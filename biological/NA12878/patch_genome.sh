...
#minimap2 -x asm20 -t 24 $REFERENCE_FASTA $PREFIX.patched.fasta > $PREFIX.patched.$REFERENCE_NAME.paf 2>> $PREFIX.GPatch.err
...
#$SCRIPTS_PATH/find_contig_breakpoints.py -c $PREFIX.contigs.bed -p $PREFIX.patched.$REFERENCE_NAME.paf -d $CBREAK_MAXCDIST -q $CBREAK_MAXQDIST -m $CBREAK_MAXDIST -s $CBREAK_MINSIZE -a > $PREFIX.breakpoints.txt
#echo "Breaking contigs at suspicious rearrangement breakpoints..."
#$SCRIPTS_PATH/break_contigs.py -f $ASSEMBLY_FASTA -b $PREFIX.breakpoints.txt > $PREFIX.cbreak.fa
...
#minimap2 -x asm20 -t 24 $REFERENCE_FASTA $PREFIX.cbreak.patched.fasta > $PREFIX.cbreak.patched.$REFERENCE_NAME.paf 2>> $PREFIX.GPatch.err
...
# Second round of contig-breaking and patching
echo "Finding suspicious rearrangement breakpoints, round 2..."
$SCRIPTS_PATH/find_contig_breakpoints.py -c $PREFIX.cbreak.contigs.bed -p $PREFIX.cbreak.patched.$REFERENCE_NAME.paf -d $CBREAK_MAXCDIST -q $CBREAK_MAXQDIST -m $CBREAK_MAXDIST -s $CBREAK_MINSIZE -a > $PREFIX.cbreak_2.breakpoints.txt
echo "Breaking contigs at suspicious rearrangement breakpoints, round 2..."
$SCRIPTS_PATH/break_contigs.py -f $PREFIX.cbreak.fa -b $PREFIX.cbreak_2.breakpoints.txt > $PREFIX.cbreak_2.fa

echo "Remapping to reference..."
minimap2 -x asm20 -t 24 -a $REFERENCE_FASTA $PREFIX.cbreak_2.fa | samtools view -b - > $PREFIX.cbreak_2.bam 2>> $PREFIX.GPatch.err

echo "Stage three genome patching..."
if [ "${WHITELIST}" != "" ]; then
    /usr/bin/time -v $PG_PATH/GPatch.py -q $PREFIX.cbreak_2.bam -r $REFERENCE_FASTA -x $PREFIX.cbreak_2 -w $WHITELIST $GPATCH_ARGS 2>> $PREFIX.GPatch.err
else
    /usr/bin/time -v $PG_PATH/GPatch.py -q $PREFIX.cbreak_2.bam -r $REFERENCE_FASTA -x $PREFIX.cbreak_2 $GPATCH_ARGS 2>> $PREFIX.GPatch.err
fi

# Stats and dot plots
printf "post-break_2 contig base count: %d\n" $(awk 'BEGIN {len=0} {len += ($3-$2)} END {print len}' $PREFIX.cbreak_2.contigs.bed) >> $PREFIX.stats 2>> $PREFIX.GPatch.err
printf "post-break_2 patch base count: %d\n" $(awk 'BEGIN {len=0} {len += ($3-$2)} END {print len}' $PREFIX.cbreak_2.patches.bed) >> $PREFIX.stats 2>> $PREFIX.GPatch.err
printf "post-break_2 total patched genome size (bp): %d\n" $(grep -v ">" $PREFIX.cbreak_2.patched.fasta | awk 'BEGIN{N=0}{N+=length($0)}END{print N}') >> $PREFIX.stats 2>> $PREFIX.GPatch.err

# Final alignment to reference and dot-plots.
echo "Mapping the final patched genome against the reference..."
minimap2 -x asm20 -t 24 $REFERENCE_FASTA $PREFIX.cbreak_2.patched.fasta > $PREFIX.cbreak_2.patched.$REFERENCE_NAME.paf 2>> $PREFIX.GPatch.err
echo "Generating dot plots"
$SCRIPTS_PATH/create_dotplots.Rscript $PREFIX.cbreak_2.patched.$REFERENCE_NAME.paf $PREFIX.cbreak_2 $REFERENCE_NAME 2>> $PREFIX.GPatch.err
